...
一款以「回憶」為主題、在瀏覽器中直接執行的 **3D 滑動拼圖遊戲**，採用 Three.js 打造沉浸式立體視覺效果。玩家可上傳自己的照片，或直接選擇首頁預設圖，系統會將圖片切割成若干碎片並打亂排列，玩家需要透過交換來還原完整影像。
...
| 🖼️ **預設照片快速開始** | 首頁提供兩排預設圖片：第一排從 `1~8` 隨機 4 張且同一小時內固定不變（整點更新），第二排從 `generated-1~10` 每次進入首頁都隨機 4 張 |
...
| 🚪 **快速退出** | 遊戲畫面右上角提供固定「退出」圖示，需確認後才會退出並回到首頁 |
...
隨機打亂碎片位置
      │
      ▼
玩家點擊兩塊碎片或按住拖曳到目標碎片 → Raycaster 偵測目標碎片 → TWEEN 動畫交換位置
      │
      ▼
...
1. **開啟遊戲頁面**，看到進場說明畫面。
2. 準備圖片可用兩種方式擇一：
   - **點擊「選擇或拍攝照片」**，從裝置選取一張您喜歡的圖片（桌機可開啟檔案選擇器，行動裝置可從相簿挑選或直接拍攝）。
   - **點選首頁下方預設圖片**：
     - 第一排：從 `images/1.png ~ images/8.png` 隨機 4 張，且同一小時內不變（整點後更新）。
     - 第二排：從 `images/generated-1.png ~ images/generated-10.png` 隨機 4 張，每次進入首頁都會重抽。
3. 選完圖片後，系統會自動**載入並切割圖片**，並先自動顯示一次「查看原圖」。
4. 點擊關閉預覽後遊戲正式開始，畫面上方顯示**計時器**與**移動次數**。
5. 交換拼圖支援兩種方式：
   - **點擊第一塊，再點擊第二塊**，即可交換位置。
   - **按住（或觸碰）第一塊並拖曳到第二塊後放開**，也可直接交換。
   - 每次交換都會計入一次移動次數。
6. 若需要提示，可按下畫面下方的**「查看原圖」**按鈕，半透明原圖將疊加顯示，再次點擊則關閉。
7. 當所有碎片回到正確位置後，遊戲自動偵測**通關**，播放粒子特效並顯示成績（時間與移動次數）。
8. 若要中途離開，可點擊畫面**右上角退出圖示**，確認後返回首頁。
9. 點擊「**重新創造回憶**」可返回首頁，重新上傳圖片或再次挑戰。
...
├── images/         # 首頁預設圖片（1~8 與 generated-1~10）
